...
datablock ItemData(RPChaingun)
{
   ...
   friction     = 0.6;
   pickupRadius = 2;
   pickUpName   = "A M32 Automatic Rifle";
...
datablock ItemData(HRPChaingun)
{
   ...
   friction     = 0.6;
   pickupRadius = 2;
   pickUpName   = "a M32 w Grenade Launcher";
...
function WeaponCheckMag(%obj,%data){
   if(%obj.inv[%data.magtype] > 0){
	%obj.clipReloading = true;
	if(%data.magImage !$= "")
	   %obj.unmountImage(%data.magImageSlot);
	%obj.Reloading = schedule(%data.magReloadTime, 0, "WeaponReloadMag", %obj, %data);
   }
}

function WeaponReloadMag(%obj,%data){
   %obj.clipreloading = false;
   %obj.decInventory(%data.magtype,1);
   if(%data.magImage !$= "")
	%obj.mountImage(%data.magImage,%data.magImageSlot);
   %obj.setInventory(%data.ammo,%data.magSize);
} 