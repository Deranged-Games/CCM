...
datablock ItemData(flamer)
{
   ...
   friction = 0.6;
   pickupRadius = 2;
	pickUpName = "a M65 flame thrower";
};
...
function FlameboltMain::onCollision(%data, %projectile, %targetObject, %modifier, %position, %normal){
   if(isObject(%targetObject)){
      %type = %targetObject.getDataBlock().getClassName();
      if(%type $= "PlayerData")
         %targetObject.maxfirecount = %targetObject.maxfirecount + 50;
      else if(!(%type $= "FlyingVehicleData" || %type $= "HoverVehicleData" || %type $= "WheeledVehicleData"))
         FlameAreaBurnLoop(%projectile,%position,%projectile.sourceObject,1);
   }
   Parent::onCollision(%data, %projectile, %targetObject, %modifier, %position, %normal);
}

function FlameAreaBurnLoop(%expsource,%pos,%sourceobj,%firecount){
   if(%firecount <= 100){
      RadiusExplosion(%expsource, %pos, "5", 0.01, 0, %sourceObj, $DamageType::Plasma);
      %fire = new ParticleEmissionDummy(){ 
      ...
      %fire.schedule(100, "delete");
      %firecount++;
      schedule(100, 0, "FlameAreaBurnLoop", %expsource,%pos,%sourceobj,%firecount);
   }
}