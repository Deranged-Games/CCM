...
datablock PlayerData(MediumMaleHumanArmor) : MediumPlayerDamageProfile
{
   ...
   maxMines = 1;              // Max number of different mines the player can have
	...
	max[PBCAmmo]			= 0;
	max[recoilless]			= 1;
	max[recoillessAmmo]			= 3;
	max[recoillessClip]			= 4;
	max[M9]				= 1;
	max[SM9]			= 0;
	max[revolver]			= 0;
	max[deagle]			= 1;
...
datablock PlayerData(ZombieArmor) : LightMaleHumanArmor
{
   ...
   damageScale[$DamageType::sniper] = 1.8;
   damageScale[$DamageType::bazooka] = 4.0;
   damageScale[$DamageType::PBC] = 0.25;
   damageScale[$DamageType::recoilless] = 1;
   damageScale[$DamageType::Missile] = 2;
...
datablock PlayerData(FZombieArmor) : LightMaleBiodermArmor
{
   ...
   damageScale[$DamageType::sniper] = 1.6;
   damageScale[$DamageType::PBC] = 0.25;
   damageScale[$DamageType::recoilless] = 2;
   damageScale[$DamageType::Missile] = 2;
...
datablock PlayerData(LordZombieArmor) : HeavyMaleBiodermArmor
{
   ...
   damageScale[$DamageType::sniper] = 2; 
   damageScale[$DamageType::bazooka] = 3;
   damageScale[$DamageType::recoilless] = 3;
   damageScale[$DamageType::Missile] = 3;
...
datablock PlayerData(DemonZombieArmor) : LightMaleHumanArmor
{
   ...
   damageScale[$DamageType::sniper] = 2.2; 
   damageScale[$DamageType::bazooka] = 2;
   damageScale[$DamageType::recoilless] = 2;
   damageScale[$DamageType::Missile] = 2;
...
$ammoType[18] = "Pistolclip";
$ammoType[19] = "Rifleclip";
$ammoType[20] = "MGclip";
$ammoType[21] = "Shotgunclip";
$ammoType[22] = "RShotgunclip";
$ammoType[23] = "LMissileLauncherAmmo";
$ammoType[24] = "RPGAmmo";
$ammoType[25] = "LSMGclip";
$ammoType[26] = "PBCAmmo";
$ammoType[27] = "SRifleSGAmmo";
$ammoType[28] = "SRifleGLAmmo";
$ammoType[29] = "recoillessAmmo";
$ammoType[30] = "recoillessclip";
...
//	error("Armor::damageObject( "@%data@", "@%targetObject@", "@%sourceObject@", "@%position@", "@%amount@", "@%damageType@", "@%momVec@" )");
   ...
   if (%damageType == $DamageType::ZAcid && %armortype !$= "ZombieArmor" && %armortype !$= "FZombieArmor" && %armortype !$= "LordZombieArmor" && %armortype !$= "DemonZombieArmor" && %armortype !$= "DemonMotherZombieArmor" && %armortype !$= "RapierZombieArmor" && %targetobject.infected != 1){
   ...
   if(%targetObject.getState() $= "Dead")
   {
	if(isObject(%targetObject.beacon))
      ...
      %damLoc = %targetObject.getDamageLocation(%position);
      Game.onClientKilled(%targetClient, %sourceClient, %damageType, %sourceObject, %damLoc, %targetobject);
   }
   else if ( %amount > 0.1 )
   {
...
function Armor::applyConcussion( %this, %dist, %radius, %sourceObject, %targetObject )
{
   ...
   if( %random <= %probabilityItem )
   {
	...
	if( %weaps[14] = %player.getInventory("Nukeme") > 0 ) %numWeapons++;
	...
	if( %weaps[25] = %player.getInventory("LMissileLauncher") > 0 ) %numWeapons++;
      ...
      if( %weaps[30] = %player.getInventory("Recoilless") > 0 ) %numWeapons++;
