...
function Beacon::onUse(%data, %obj)
{
   ...
   if(!%searchResult) 
   {
   	%weapdata = %obj.getMountedImage($weaponslot); 
	if(%weapdata.magType !$= ""){
	   if(%obj.inv[%weapdata.ammo] < %weapdata.magSize){
		if(%obj.inv[%weapdata.magType] > 0){
		   %obj.Reloading = weaponCheckMag(%obj,%weapdata);
		   %obj.setInventory(%weapdata.ammo,0);
		   return 0;
		}
	   }
	}
