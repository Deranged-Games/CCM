		...
		if (%this.usingHooker == 1 && getSimTime() > (%this.grenadeModeTime + 100) && %this.performing == 0) {
		   %this.grenadeModeTime = getSimTime();
               if (%this.HookerMode == 2 || %this.HookerMode $= ""){
                  %this.HookerMode = 1;
			bottomPrint(%this.client,"Grappling Tool set to Climb",2,1);
		   }
		   else if (%this.HookerMode == 1){
			%this.HookerMode = 2;
			bottomPrint(%this.client,"Grappling Tool set to Rappel",2,1);
		   }
		   return;
          	}
        	//end Hooker tool
...
function ShapeBase::maxInventory(%this,%data) {
   if($TestCheats && %this.getDatablock().max[%data.getName()] !$= "")
      ...
      %max = %this.getDatablock().max[%data.getName()];
      if(%this.getMountedImage($PouchSlot).item.ammoPouch){
	 %item = %this.getMountedImage($PouchSlot).item;
         %max = %max + %item.max[%data.getName()];
      }
      return %max;
   }
}
...
function ShapeBase::clearInventory(%this)
{
   ...
   %this.setInventory(recoilless, 0);
   %this.setInventory(recoillessammo, 0);
   %this.setInventory(recoillessclip, 0);

   %this.setInventory(pistolammo, 0);
   %this.setInventory(pistolclip, 0);
   %this.setInventory(revolverAmmo, 0);
   %this.setInventory(deagleammo, 0);
   %this.setInventory(glockammo, 0);
   %this.setInventory(uziAmmo, 0);
   %this.setInventory(M9, 0);
   %this.setInventory(SM9, 0);
   %this.setInventory(Revolver, 0);
   %this.setInventory(deagle, 0);
   %this.setInventory(glock, 0);
   %this.setInventory(uzi, 0);
 
   %this.setInventory(MergeTool, 0);
   %this.setInventory(HookTool, 0);
   ...
   // take away any pack the player has
   %curPack = %this.getMountedImage($PouchSlot);
   if(%curPack > 0)
      %this.setInventory(%curPack.item, 0);
...
function serverCmdGiveAll(%client)
{
   if($TestCheats)
   {
	...
	%player.setInventory(LMissileLauncher,999);
      ...
      %player.setInventory(recoillessammo, 999);
      %player.setInventory(recoillessclip, 999);

      %player.setInventory(pistolammo, 999);
      %player.setInventory(pistolclip, 999);
      %player.setInventory(revolverAmmo, 999);
      %player.setInventory(deagleammo, 999);
      %player.setInventory(glockammo, 999);
      %player.setInventory(uziAmmo, 999);
      %player.setInventory(M9, 1);
      %player.setInventory(SM9, 1);
      %player.setInventory(Revolver, 1);
      %player.setInventory(deagle, 1);
      %player.setInventory(glock, 1);
      %player.setInventory(uzi, 1);


      %player.setInventory(MergeTool, 1);
      %player.setInventory(HookTool, 1);
