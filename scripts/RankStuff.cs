...
$Ranks::MinPoints[15] = 27500;
$Ranks::NewRank[15] = "Major General";
$Ranks::MinPoints[16] = 35000;
$Ranks::NewRank[16] = "Lieutenant General";
$Ranks::MinPoints[17] = 45000;
$Ranks::NewRank[17] = "General";
$Ranks::MinPoints[18] = 75000;
	...
	for(%j = 18; %j > 0; %j--){
	   if($Rank::Score[%cl.ranknum] >= $Ranks::MinPoints[%j]){
		if($Rank::Rank[%cl.ranknum] !$= $Ranks::NewRank[%j]){
		   ...
		   messageAll('msgClient',"\c2"@%name@" has become a "@$Ranks::NewRank[%j]@" with a score of "@$Rank::Score[%cl.ranknum]@"!");
		   bottomPrint(%cl, "Congradulations "@%name@" you have been promoted to the rank of: "@$Ranks::NewRank[%j]@"!", 5, 2 );
		}
		%j = 1;
	   }
	}
...
function ccCheckStats(%client, %args){
   if(%args $= ""){
	...
	messageClient(%client, 'MsgClient', "\c2Your Rank is "@%Rank@" and your point standing is "@%stats@" your next rank is "@%nextrank@" and you need "@(%nextrankscore - %stats)@" points.");
   }
   else{
	%count = ClientGroup.getCount();
	for(%i = 0; %i < %count; %i++){
	   ...
	   if(%cl.nameBase $= %args){
		...
		messageClient(%client, 'MsgClient', "\c2"@%args@" Rank is "@%Rank@" and "@%args@" point standing is "@%stats@".");
		%i = %count;
	   }
	   else if(%i == (%count - 1))
...
function ccTop5(%client,%args){
   ...
   messageClient(%client, 'MsgClient', "\c2Top 5 Players are:");
   for(%i = 1; %i <= 5; %i++){
	messageClient(%client, 'MsgClient', "\c2"@%i@": "@$Rank::Top[%i]@".");
   }
}
...
function cccreatesquad(%sender, %args){
   ...
   if($Rank::Score[%sender.ranknum] < $Ranks::MinPoints[4]){
	messageclient(%sender, 'MsgClient', '\c2You are not high enough rank to create a squad.');
	return;
   }
   if(%args $= ""){
   ...
   messageclient(%sender, 'MsgClient', "\c2Squad "@getWord(%args,0)@" has been created.");
...
function ccJoin(%sender,%name){
   ...
   for(%i = 0; %i < $squad::numsquads; %i++){
	if(%name $= $squad::Name[%i]){
	   ...
	   messageclient(%sender, 'MsgClient', "\c2You are now a part of squad "@%name@".");
	   return;
	}
   }
   messageclient(%sender, 'MsgClient', "\c2Squad "@%name@" Dosent exist.");
}
...
function ccInvite(%sender, %args){
   ...
   for(%i = 0; %i < %count; %i++){
	...
	if(%cl.nameBase $= %args)
	   ...
	   messageClient(%sender, 'MsgClient', "\c2"@%args@" is not a valid player.");
	   return;
	}
   }
   if($Rank::Squad[%cl.ranknum] !$= ""){
	messageClient(%sender, 'MsgClient', "\c2"@%args@" is already in a squad.");
	return;
   }
   for(%i = 0; %i < $squad::numsquads; %i++){
	if(%sender.namebase $= $squad::Leader[%i]){
	   %cl.sqinv = $squad::Name[%i];
	   messageclient(%sender, 'MsgClient', '\c2Has been invited to your squad.');
	   messageclient(%cl, 'MsgClient', "\c2You have been invited to squad "@$squad::Name[%i]@" say /join to join.");
	   return;
	}  
   }
   ...
   for(%i = 0; %i < %count; %i++){
	...
	while($squad::member[%num] !$= ""){
	   if(%cl.ranknum == $squad::member[%num] && %cl.team == %sender.team)
		messageclient(%cl, 'MsgClient', "\c0"@$Rank::rank[%sender.ranknum]@": \c2"@%sender.namebase@" : \c1"@%args);
	   %mem++;
	   %num = getWord($Rank::Squad[%sender.ranknum],1)@%mem;
	}
   }
...
function ccListSquads(%client, %args){
   for(%i = 0; %i < $Squad::numsquads; %i++){
	messageClient(%client, 'MsgClient', "\c2SQUAD"@(%i + 1)@": \c1"@$squad::Name[%i]@" \c2SCORE: \c1"@$squad::score[%i]@".");
	%temp = 0;
	%num = %i@%temp;
	while($squad::member[%num] !$= ""){
	   if(%temp == 0)
	   ...
	   messageClient(%client, 'MsgClient', "\c2----"@%prefix@" \c1"@$rank::Name[$squad::member[%num]]@" \c2teamscore: \c1"@$rank::squadscore[$squad::member[%num]]@".");
	   %temp++;
	   %num = %i@%temp;
	}
...
function ccRequestInvite(%client, %args){
   ...
   for(%i = 0; %i < $squad::numsquads; %i++){
	if(%args $= $squad::Name[%i]){
	   messageclient(%sender, 'MsgClient', "\c2Request was sent to squad "@%args@".");
	   for(%j = 0; %j < %count; %j++){
		%cl = ClientGroup.getObject(%j);
		if(%cl.namebase $= $squad::Leader[%i])
		   messageclient(%cl, 'MsgClient', "\c2"@%client.namebase@" has requested to join your squad.");
	   }
	   return;
	}
   }
   messageclient(%sender, 'MsgClient', "\c2Squad "@%args@" Dosent exist.");
}
...
function ccO(%sender,%args){
   ...
   if($Rank::Score[%sender.ranknum] < $Ranks::MinPoints[12]){
	messageclient(%sender, 'MsgClient', '\c2You are not high enough rank to give orders.');
	return;
   }
   ...
   if(%searchResult){
	...
	while($squad::member[%num] !$= ""){
	   ...
	   for(%i = 0; %i < %count; %i++){
		%cl = ClientGroup.getObject(%i);
		if(%cl.ranknum == $squad::member[%num] && %cl.team == %sender.team)
		   messageclient(%cl, 'MsgClient', "\c2A general has ordered your squad to "@%order@" this location.");
	   }
	   %mem++;
	   %num = %j@%mem;
	}
...
function ccforce(%sender, %args){
   ...
   if(%cmd $= "join"){
	...
	if($Rank::Score[%cl.ranknum] >= $Ranks::MinPoints[8] && !($Rank::Score[%cl.ranknum] < $Ranks::MinPoints[14] && $Rank::Score[%sender.ranknum] >= $Ranks::MinPoints[14])){
	   messageclient(%sender, 'MsgClient', '\c2You may not force this player, he has to high of a rank.');
	   return;
	}
      for(%i = 0; %i < $squad::numsquads; %i++){
	   if(%squad $= $squad::Name[%i]){
		...
		messageclient(%sender, 'MsgClient', "\c2"@%target@" has joined squad "@%squad@".");
		messageclient(%cl, 'MsgClient', '\c2Higher Rank forced:');
		ccJoin(%cl, %squad);
	      return;
	   }
	...
	messageclient(%sender, 'MsgClient', "\c2Squad "@%squad@" has been created with "@%target@" as leader.");
	if($Rank::Squad[%cl.ranknum] !$= ""){
	   messageclient(%cl, 'MsgClient', '\c2Higher Rank forced:');
	   ccLeaveSquad(%cl,"");
	   messageclient(%cl, 'MsgClient', '\c2 ');
	...
	for(%i = 0; %i < %count; %i++){
	   ...
	   if(%cl.namebase $= %target)
		...
		messageClient(%client, 'MsgClient', "\c2"@%target@" is not a valid player.");
		return;
	   }
	}
	if($Rank::Score[%cl.ranknum] >= $Ranks::MinPoints[8] && !($Rank::Score[%cl.ranknum] < $Ranks::MinPoints[14] && $Rank::Score[%sender.ranknum] >= $Ranks::MinPoints[14])){
	...
	if($Rank::Squad[%cl.ranknum] !$= ""){
	   messageclient(%cl, 'MsgClient', '\c2Higher Rank forced:');
	   ccLeaveSquad(%cl,"");
	   messageclient(%sender, 'MsgClient', '\c2'@%target@' has been forced out of his squad.');
	}
	else 
