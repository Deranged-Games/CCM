...
function cycleVehicleHud(%obj, %client,%data) {
	...
	if ( %station.vehicle[FFTransport] ) {
		%vehicleSet[FFTransport] = %i;
		%i++;
	}
	...
	if ( checkVehSet(%vehicleSet[scoutVehicle], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Wildcat Cycle", "", ScoutVehicle, $VehicleMax[ScoutVehicle] - $VehicleTotalCount[%team, ScoutVehicle] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[AssaultVehicle], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Wolf Light Tank", "", AssaultVehicle, $VehicleMax[AssaultVehicle] - $VehicleTotalCount[%team, AssaultVehicle] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[CGTank], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Banshee Strike Tank", "", CGTank, $VehicleMax[CGTank] - $VehicleTotalCount[%team, CGTank] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[HeavyTank], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Faustes Heavy Tank", "", HeavyTank, $VehicleMax[HeavyTank] - $VehicleTotalCount[%team, HeavyTank] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[FFTransport], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Rover APC", "", FFTransport, $VehicleMax[FFTransport] - $VehicleTotalCount[%team, FFTransport] );
		%count++;
	}
	...
	if ( checkVehSet(%vehicleSet[Artillery], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Harbinger Artillery Platform", "", Artillery, $VehicleMax[Artillery] - $VehicleTotalCount[%team, Artillery] );
		%count++;
	}
	...
	if ( checkVehSet(%vehicleSet[scoutFlyer], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Raptor II Interceptor", "", ScoutFlyer, $VehicleMax[ScoutFlyer] - $VehicleTotalCount[%team, ScoutFlyer] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[StrikeFlyer], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Awring Strike Fighter", "", StrikeFlyer, $VehicleMax[StrikeFlyer] - $VehicleTotalCount[%team, StrikeFlyer] );
		%count++;
	}
	...
	if ( checkVehSet(%vehicleSet[bomberFlyer], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Shamshire Bomber", "", BomberFlyer, $VehicleMax[BomberFlyer] - $VehicleTotalCount[%team, BomberFlyer] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[gunship], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "AC-290 Gunship", "", gunship, $VehicleMax[gunship] - $VehicleTotalCount[%team, gunship] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[hapcFlyer], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Havoc Transport", "", HAPCFlyer, $VehicleMax[HAPCFlyer] - $VehicleTotalCount[%team, HAPCFlyer] );
		%count++;
	}
	...
	if ( checkVehSet(%vehicleSet[HeavyChopper], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Eagle VII Chopper", "", HeavyChopper, $VehicleMax[HeavyChopper] - $VehicleTotalCount[%team, HeavyChopper] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[Personelboat], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Landing boat", "", Personelboat, $VehicleMax[Personelboat] - $VehicleTotalCount[%team, Personelboat] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[boat], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Hercules Gun Boat", "", boat, $VehicleMax[boat] - $VehicleTotalCount[%team, boat] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[sub], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Darwin Submarine", "", sub, $VehicleMax[sub] - $VehicleTotalCount[%team, sub] );
		%count++;
	}
...
$vehicleReticle[CGTank, 1, bitmap] = "gui/hud_ret_tankchaingun";
$vehicleReticle[CGTank, 1, frame] = true;
$vehicleReticle[CGTank, 2, bitmap] = "gui/hud_ret_tankmortar";
$vehicleReticle[CGTank, 2, frame] = true;

$vehicleReticle[AWACS, 1, bitmap] = "gui/ret_missile";
$vehicleReticle[AWACS, 1, frame] = false;
$vehicleReticle[AWACS, 2, bitmap] = "";
$vehicleReticle[AWACS, 2, frame] = false;
$vehicleReticle[AWACS, 3, bitmap] = "";
$vehicleReticle[AWACS, 3, frame] = false;