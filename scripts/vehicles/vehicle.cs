...
//   %data.canObserve = true;
   ...
   if(%obj.getdatablock().getname() $= "scoutFlyer" || %obj.getdatablock().getname() $= "strikeFlyer" || %obj.getdatablock().getname() $= "SuperiorityFighter")
	checkStallLoop(%obj);
}
...
function VehicleData::onDestroyed(%data, %obj, %prevState) {
   ...
   for(%i = 0; %i < %obj.getDatablock().numMountPoints; %i++)
   {
      if (%obj.getMountNodeObject(%i)) {
         ...
         %flingee.damage(0, %obj.getPosition(), 1.5, $DamageType::Crash);
      }
   }
//[[CHANGE]]
   if (%obj.clientControl)
