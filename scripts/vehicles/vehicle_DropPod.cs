...
function MakeDropPod(%pos, %team){
   ...
   %pod.setInventory(PlasmaAmmo, 3);
   return %pod;
}

function DropPod::playerMounted(%data, %obj, %player, %node)
{
   bottomPrint(%player.client, "Use /SetCoord and /LaunchPod to launch the Drop Pod", 5, 2 );
...
function ccSetCoord(%sender, %args){
   ...
   if(%plyr.mountedToV){
	...
	if(%veh.getDataBlock().getName() $= "DropPod"){
	   ...
	   messageclient(%sender, 'MsgClient', "\c5Drop Pod Coord set to "@%veh.DPcoord@".");
	   ...
	   messageclient(%sender, 'MsgClient', '\c5You are not in a Drop Pod.');
   }
   else
	messageclient(%sender, 'MsgClient', '\c5You are not in a Drop Pod.');
}
   ...
   if(%plyr.mountedToV){
	...
	if(%veh.getDataBlock().getName() $= "DropPod"){
	   if(%veh.DPcoord !$= "")
		...
		messageclient(%sender, 'MsgClient', '\c5Please set the coordinates at which you wish to land with /setcoord.');
	}
      else
	   messageclient(%sender, 'MsgClient', '\c5You are not in a Drop Pod.');
   }
...
function droppod::onTrigger(%data, %obj, %trigger, %state){
   if (%trigger ==4){
      switch (%state){
         case 1:
