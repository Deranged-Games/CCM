...
function HammerATlockon(%p,%pos2)
{ 
   if(!isObject(%p))
	return;
   %pos = %p.getPosition();
   if(%pos2){
      ...
      %tpos = vectorscale(%vec, 70);
      %tpos = vectoradd(%pos,%tpos);

      InitContainerRadiusSearch(%tpos, 65, $TypeMasks::VehicleObjectType);
   ...
   %pos2 = %pos;
   %p.HATlockon = schedule(100, 0, "HammerATlockon", %p,%pos2);
}
...
function gunship::onTrigger(%data, %obj, %trigger, %state){
   if (%trigger ==4){
      switch (%state){
         case 1:
