...
function SubTorpedoLauncher::onFire(%data,%obj,%slot)
{
   ...
   %p.torpseekloop = schedule(1000, 0, "TorpedoSeekLoop", %p);
}
...
function TorpedoSeekLoop(%p,%pos2)
{ 
   if(!isObject(%p))
	return;
   %pos = %p.getPosition();
   if(%pos2 != 0){
      ...
      %tpos = vectorscale(%vec, 65);
      %tpos = vectoradd(%pos,%tpos);

      InitContainerRadiusSearch(%tpos, 55, $TypeMasks::VehicleObjectType);
   ...
   %pos2 = %pos;
   %p.torpseekloop = schedule(500, 0, "TorpedoSeekLoop", %p,%pos2);
}
