...
function TargetingLaserImage::onFire(%data,%obj,%slot)
{
   ...
   %obj.LTLpos = "0 0 0";
   callArtillerycheck(%obj,%p); 
}

function callArtillerycheck(%obj,%p,%count){
   if(!isObject(%obj))
	return;
   if(!isObject(%p))
	return;
   bottomPrint(%obj.client, "Hold laser in one point for 5 seconds to call for artillery.", 5, 2 );
   %pos = %obj.lastProjectile.getTargetPoint();
   if(vectorDist(%pos,%obj.LTLpos) > 5)
	%count = 0;
   else
 	%count++;
   %obj.LTLpos = %pos;
   if(%count == 5){
	...
	callInArtillery(%beacon);
	%obj.LTLpos = "";
	return;
   }
   schedule(1000, 0, "callArtillerycheck",%obj,%p,%count);
}
