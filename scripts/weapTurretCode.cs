...
function FlareGrenade::onUse(%this, %obj)
{
   if(Game.handInvOnUse(%data, %obj)) {
      ...
      schedule(2000, 0, "Pflareinit", %p);
      %obj.lastThrowTime[%data] = getSimTime();
      %obj.throwStrength = 0;
   }
}

function pflareinit(%p){
   if (!isObject(%p)) 
      return; 
   %pn = new (FlareProjectile)() { 
      dataBlock        = ParachuteFlare; 
      initialDirection = "0 0 -1"; 
   ...
   FlareSet.add(%pn);
   %P.delete(); 
   %pn.schedule(30000, "delete");
//   schedule(250, 0, "pflaresparks", %pn, 1);
}

function pflaresparks(%p,%count){
   if (!isObject(%p)) 
      return; 
   if(%count >= 50)
      return;
   ...
   for(%x = 0; %x < %Rand; %x++){
      %pn = new (FlareProjectile)() { 
         dataBlock        = ParachuteFlare; 
         initialDirection = (getRandom() - 0.5) SPC (getRandom() - 0.5) SPC "-1"; 
         initialPosition  = %p.getPosition(); 
         sourceObject     = %p.sourceObject; 
         sourceSlot       = %p.sourceSlot; 
      }; 
      MissionCleanup.add(%pn); 
      FlareSet.add(%pn);
      %pn.schedule(15000, "delete");
   }
   schedule(250, 0, "pflaresparks", %p, %count++);
}
