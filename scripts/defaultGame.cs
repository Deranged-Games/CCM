...
function DefaultGame::spawnPlayer( %game, %client, %respawn ) {
   if($Rank::Squad[%client.ranknum] !$= "" && %client.spawnOnLead == 1 && $Host::Purebuild == 0){
      ...
      if(%leader.team == %client.team && isObject(%leader.player)){
	   ...
	   %pos = vectorAdd(%client.SP.getPosition(),vectorScale(%client.SP.getUpVector(),-1.4));
	   %pos = vectorAdd(%pos,"0 0 -1.3");
...
// called from player scripts
function DefaultGame::onClientKilled(%game, %clVictim, %clKiller, %damageType, %implement, %damageLocation, %victim)
{
...
//[[CHANGE]] Make sure the beacon get's removed.. as it should be.. :D
   ...
   if (%damageType == $DamageType::Suicide)
   {
	...
	%game.DynamicZpoints(%clkiller,%victim,%implement);
}

function DefaultGame::DynamicZpoints(%game,%killer,%victim,%implement){
...
function DefaultGame::displayDeathMessages(%game, %clVictim, %clKiller, %damageType, %implement)
{
   ...
   {
      if((%damageType == 56 || %damageType == 6 || %damageType == 83) && (%clVictim.headShot))
      {
...
function DefaultGame::clientMissionDropReady(%game, %client)
{
   ...
   centerPrint(%client, "Welcome to Combat Construction Mod "@%client.nameBase@".", 3);
}
