...
function SpawnPointImage::onDeploy(%item,%plyr,%slot) {
	...
	if($teamSPs[%deplObj.team] $= "")
	   $TeamSPs[%deplObj.team] = 0;
	$teamSP[%deplObj.team,$teamSPs[%deplObj.team]] = %deplObj;
	$teamSPs[%deplObj.team]++;
	if(%plyr.client.SPname $= "")
	   %deplObj.name = getWords(%position,0,1);
	else
	   %deplObj.name = %plyr.client.SPname;
	%piece.active = 0;
