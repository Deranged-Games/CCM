...
function zombieSpawnLoop(%pos, %radius, %freq, %loopnum){
   if(%loopnum $= ""){
      %num = 1;
      while($zombieloop[%num] !$= ""){
         %num++;
      }
      %loopnum = %num;
   }
   ...
   $zombieloop[%loopnum] = schedule(500, 0, "zombieSpawnLoop", %pos, %radius, %freq, %loopnum);
}
